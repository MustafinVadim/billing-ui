...
import { handleAction, createAction, createActions, combineActions } from "../../../libs/redux-actions";
...
describe("handleAction()", () => {
    ...
    it("uses the identity if the specified reducer is undefined", () => {
        ...
        expect(reducer(prevState, { type })).to.deep.equal(prevState);
        expect(reducer(prevState, { type, error: true, payload: new Error })).to.deep.equal(prevState);
    });
    ...
    describe("map of handlers form", () => {
        ...
        describe("resulting reducer", () => {
            ...
            it("returns previous state if matching handler is not function", () => {
                ...
                expect(reducer(prevState, { type, payload: 123 })).to.deep.equal(prevState);
                expect(reducer(prevState, { type, payload: 123, error: true })).to.deep.equal(prevState);
            });
        });
    });
    ...
    describe("with combined actions", () => {
        it("should handle combined actions in reducer form", () => {
            ...
            expect(reducer({ number: 1 }, action1(1)))
                .to.deep.equal({ ...defaultState, number: 2 });
            expect(reducer({ number: 1 }, { type: "ACTION_2", payload: 2 }))
                .to.deep.equal({ ...defaultState, number: 3 });
            expect(reducer({ number: 1 }, { type: "ACTION_3", payload: 3 }))
                .to.deep.equal({ ...defaultState, number: 4 });
        });
        ...
        it("should return previous state if action is not one of the combined actions", () => {
            ...
            expect(reducer(state, { type: "ACTION_3", payload: 1 })).to.deep.equal({ ...defaultState, ...state });
        });
        ...
        it("should handle combined actions with symbols", () => {
            ...
            expect(reducer({ number: 0 }, action1(1)))
                .to.deep.equal({ ...defaultState, number: 1 });
