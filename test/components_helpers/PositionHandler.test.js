...
describe("Position handler", () => {
    ...
    describe("adjustPositionType", () => {
        ...
        it("adjustPositionType не поместился вниз с типом tip", () => {
            ...
            let positionType = adjustPositionType(PositionType.bottomLeft, target, tooltip, TooltipType.tip, mainWrapper, container);
            ...
            positionType = adjustPositionType(PositionType.leftTop, target, tooltip, TooltipType.tip, mainWrapper, container);
            ...
            positionType = adjustPositionType(PositionType.leftMiddle, target, tooltip, TooltipType.tip, mainWrapper, container);
            expect(positionType).to.equal(PositionType.leftBottom);

            positionTarget.top = mainWrapper.clientHeight - positionTarget.height / 2 - tooltip.offsetHeight + ARROW_VERTICAL_MARGIN + ARROW_HEIGHT + 1;
            positionType = adjustPositionType(PositionType.rightTop, target, tooltip, TooltipType.tip, mainWrapper, container);
            expect(positionType).to.equal(PositionType.rightBottom);

            positionTarget.top = mainWrapper.clientHeight - positionTarget.height / 2 - tooltip.offsetHeight / 2 + 1;
            positionType = adjustPositionType(PositionType.rightMiddle, target, tooltip, TooltipType.tip, mainWrapper, container);
            expect(positionType).to.equal(PositionType.rightBottom);
        });
        ...
        it("adjustPositionType не поместился вниз c типом validation", () => {
            ...
            let positionType = adjustPositionType(PositionType.bottomLeft, target, tooltip, TooltipType.validation, mainWrapper, container);
            expect(positionType).to.equal(PositionType.topLeft);
            ...
            positionType = adjustPositionType(PositionType.leftTop, target, tooltip, TooltipType.validation, mainWrapper, container);
            expect(positionType).to.equal(PositionType.leftBottom);

            positionTarget.top = mainWrapper.clientHeight - positionTarget.height - tooltip.offsetHeight / 2 + 1;
            positionType = adjustPositionType(PositionType.leftMiddle, target, tooltip, TooltipType.validation, mainWrapper, container);
            expect(positionType).to.equal(PositionType.leftBottom);

            positionTarget.top = mainWrapper.clientHeight - positionTarget.height - tooltip.offsetHeight + ARROW_VERTICAL_MARGIN + ARROW_HEIGHT + 1;
            positionType = adjustPositionType(PositionType.rightTop, target, tooltip, TooltipType.validation, mainWrapper, container);
            expect(positionType).to.equal(PositionType.rightBottom);

            positionTarget.top = mainWrapper.clientHeight - positionTarget.height - tooltip.offsetHeight / 2 + 1;
            positionType = adjustPositionType(PositionType.rightMiddle, target, tooltip, TooltipType.validation, mainWrapper, container);
            expect(positionType).to.equal(PositionType.rightBottom);
        });
        ...
        it("adjustPositionType не поместился вверх с типом tip", () => {
            ...
            let positionType = adjustPositionType(PositionType.topLeft, target, tooltip, TooltipType.tip, mainWrapper, container);
            ...
            positionType = adjustPositionType(PositionType.leftBottom, target, tooltip, TooltipType.tip, mainWrapper, container);
            ...
            positionTarget.top = tooltip.offsetHeight - ARROW_VERTICAL_MARGIN - ARROW_HEIGHT - positionTarget.height / 2 - 1;
            positionType = adjustPositionType(PositionType.rightBottom, target, tooltip, TooltipType.tip, mainWrapper, container);
        ...
        it("adjustPositionType не поместился вверх с типом validation", () => {
            ...
            positionTarget.top = tooltip.offsetHeight + MARGIN - 1;
            let positionType = adjustPositionType(PositionType.topLeft, target, tooltip, TooltipType.validation, mainWrapper, container);
            expect(positionType).to.equal(PositionType.bottomLeft);

            positionTarget.top = tooltip.offsetHeight - ARROW_VERTICAL_MARGIN - ARROW_HEIGHT - positionTarget.height - 1;
            positionType = adjustPositionType(PositionType.leftBottom, target, tooltip, TooltipType.validation, mainWrapper, container);
            expect(positionType).to.equal(PositionType.leftTop);
            ...
            positionTarget.top = tooltip.offsetHeight - ARROW_VERTICAL_MARGIN - ARROW_HEIGHT - positionTarget.height - 1;
            positionType = adjustPositionType(PositionType.rightBottom, target, tooltip, TooltipType.validation, mainWrapper, container);
            expect(positionType).to.equal(PositionType.rightTop);
        ...
        it("adjustPositionType не поместился слева с типом tip", () => {
            ...
            let positionType = adjustPositionType(PositionType.leftTop, target, tooltip, TooltipType.tip, mainWrapper, container);
            expect(positionType).to.equal(PositionType.rightTop);

            positionTarget.left = tooltip.offsetWidth - ARROW_RIGHT_MARGIN - positionTarget.width - 1;
            positionType = adjustPositionType(PositionType.topRight, target, tooltip, TooltipType.tip, mainWrapper, container);
            expect(positionType).to.equal(PositionType.topLeft);

            positionTarget.left = tooltip.offsetWidth / 2 - positionTarget.width / 2 - 1;
            positionType = adjustPositionType(PositionType.topCenter, target, tooltip, TooltipType.tip, mainWrapper, container);
            expect(positionType).to.equal(PositionType.topLeft);

            positionTarget.left = tooltip.offsetWidth - ARROW_LEFT_MARGIN - positionTarget.width - 1;
            positionType = adjustPositionType(PositionType.bottomRight, target, tooltip, TooltipType.tip, mainWrapper, container);
            expect(positionType).to.equal(PositionType.bottomLeft);
        ...
        it("adjustPositionType не поместился слева c типом validation", () => {
            ...
            let positionType = adjustPositionType(PositionType.leftTop, target, tooltip, TooltipType.validation, mainWrapper, container);
            expect(positionType).to.equal(PositionType.rightTop);

            positionTarget.left = tooltip.offsetWidth - positionTarget.width - 1;
            positionType = adjustPositionType(PositionType.topRight, target, tooltip, TooltipType.validation, mainWrapper, container);
            expect(positionType).to.equal(PositionType.topLeft);

            positionTarget.left = tooltip.offsetWidth / 2 - positionTarget.width / 2 - 1;
            positionType = adjustPositionType(PositionType.topCenter, target, tooltip, TooltipType.validation, mainWrapper, container);
            expect(positionType).to.equal(PositionType.topLeft);

            positionTarget.left = tooltip.offsetWidth - positionTarget.width - 1;
            positionType = adjustPositionType(PositionType.bottomRight, target, tooltip, TooltipType.validation, mainWrapper, container);
            expect(positionType).to.equal(PositionType.bottomLeft);

            positionTarget.left = tooltip.offsetWidth / 2 - positionTarget.width / 2 - 1;
            positionType = adjustPositionType(PositionType.bottomCenter, target, tooltip, TooltipType.validation, mainWrapper, container);
            expect(positionType).to.equal(PositionType.bottomLeft);
        });
        ...
        it("adjustPositionType не поместился справа с типом tip", () => {
            ...
            let positionType = adjustPositionType(PositionType.rightTop, target, tooltip, TooltipType.tip, mainWrapper, container);
            expect(positionType).to.equal(PositionType.leftTop);

            positionTarget.left = mainWrapper.clientWidth - tooltip.offsetWidth + ARROW_LEFT_MARGIN + 1;
            positionType = adjustPositionType(PositionType.topLeft, target, tooltip, TooltipType.tip, mainWrapper, container);
            ...
            positionTarget.left = mainWrapper.clientWidth - tooltip.offsetWidth + ARROW_LEFT_MARGIN + 1;
            positionType = adjustPositionType(PositionType.bottomLeft, target, tooltip, TooltipType.tip, mainWrapper, container);
        ...
        it("adjustPositionType не поместился справа c типом validation", () => {
            ...
            let positionType = adjustPositionType(PositionType.rightTop, target, tooltip, TooltipType.validation, mainWrapper, container);
            expect(positionType).to.equal(PositionType.leftTop);

            positionTarget.left = mainWrapper.clientWidth - tooltip.offsetWidth + 1;
            positionType = adjustPositionType(PositionType.topLeft, target, tooltip, TooltipType.validation, mainWrapper, container);
            expect(positionType).to.equal(PositionType.topRight);
            ...
            positionTarget.left = mainWrapper.clientWidth - tooltip.offsetWidth + 1;
            positionType = adjustPositionType(PositionType.bottomLeft, target, tooltip, TooltipType.validation, mainWrapper, container);
            expect(positionType).to.equal(PositionType.bottomRight);
